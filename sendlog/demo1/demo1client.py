...
if __name__ == '__main__':
    ...
    execfile('demo1modifi.py')
    for line in open('./log/scan.txt', 'r'):
        try:
    ...
    for line in open('./log/dir_brute.txt', 'r'):
        try:
            syslog = Syslog(host=ep_address, port=514)
            syslog.send(line, Level.ERR)
        except OSError, exc:
    ...
    for line in open('./log/sqlinject.txt', 'r'):
        try:
            syslog = Syslog(host=ep_address, port=514)
            syslog.send(line, Level.ERR)
        except OSError, exc:
    ...
    for line in open('./log/webshell.txt', 'r'):
        try:
            syslog = Syslog(host=ep_address, port=514)
            syslog.send(line, Level.ERR)
        except OSError, exc:
    ...
    for line in open('./log/ftp_brute.txt', 'r'):
        try:
            syslog = Syslog(host=ep_address, port=514)
            syslog.send(line, Level.ERR)
        except OSError, exc:
